...
def decorators_decorator(call_count: int, start_sleep_time: int, factor: int, border_sleep_time: int):
        ...
        def wrapper(*args, **kwargs):
            ...
            for count in range(2, call_count + 1):  # Запускаем цикл по количеству call_count.
                if t < border_sleep_time:  # Если время ожидания меньше конечного, увеличиваем ожидание.
                    t *= factor**count
                    if t >= border_sleep_time:  # Если время ожидания больше или равно конечному, используем последнее.
                        t = border_sleep_time
